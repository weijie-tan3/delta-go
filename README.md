...
## Fork changes (github/weijie-tan3, used by github/suez)

This is a fork of [`rivian/delta-go`](https://github.com/rivian/delta-go),
diverging at upstream `2bc9f0a`. It is consumed by
[`github/suez`](https://github.com/github/suez) via a go.mod `replace`
directive to add **ADLS Gen2 (hierarchical-namespace) support** and to fix
several places where delta-go assumes S3-style storage semantics that do not
hold on an HNS account. A future upstream rebase must carry these forward.

| Commit | Change | Why |
| ------ | ------ | --- |
| `b1510fb` | Add Azure ADLS Gen2 storage backend (`storage/azurestore`, `internal/azureutils`) | delta-go only shipped Local/S3 stores; suez writes to ADLS Gen2. |
...
| `0a9088b` | Parse the `timestamp_ntz` schema type | suez's `pushed_at` column is `timestamp_ntz`, which the schema parser didn't recognize (table is reader v3 / writer v7). |
...
The suez table is Delta reader v3 / writer v7 because of `timestamp_ntz`, but
delta-go's checkpoint caps at v1/v1. Callers opt in via
